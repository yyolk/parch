...
Vendored [Jost](https://indestructibletype.com/Jost.html) (Book / Medium / Bold / Heavy) is SIL OFL 1.1 — see `src/parch/fonts/LICENSE`. Weights stay curated. Closed roles resolve through `JostRamp` / `EffectiveRamp`. Thesis R: `[typography.overlay.<role>]` in the press TOML parses to `TypeOverlay` and merges `code defaults ⊕ device ⊕ toml` (Jost weights only; unknown keys fail). See `examples/mvp-typo-overlay.toml`. `family="jost"` stays on the ink for a later dual-font ramp.

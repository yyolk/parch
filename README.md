...
Vendored [Jost](https://indestructibletype.com/Jost.html) (Book / Medium / Bold / Heavy) is SIL OFL 1.1 — see `src/parch/fonts/LICENSE`. Weights stay curated. Closed roles resolve through `JostRamp` / `EffectiveRamp`; device and press may supply a `TypeOverlay` (size/weight only). `family="jost"` stays on the ink for a later dual-font ramp.

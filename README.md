...
parch generates **fixed e-ink PDF pages**. The MVP target is SuperNote Nomad only. Optional `outline = true` writes a PDF reader TOC at each section's first dest (cover skipped; no printed contents page).

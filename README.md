...
Vendored [Jost](https://indestructibletype.com/Jost.html) (Book / Medium / Bold / Heavy) is SIL OFL 1.1 — see `src/parch/fonts/LICENSE`. Weights stay curated. Closed roles resolve through `JostRamp` / `EffectiveRamp`. Thesis R: `[typography.overlay.<role>]` in the press TOML parses to `TypeOverlay` and merges `code defaults ⊕ device ⊕ toml` (Jost weights only; unknown keys fail). Spike job: `examples/mvp-typo-overlay.toml`. Specimens: [`exp-typeramp-toml-cover.png`](artifacts/mvp/exp-typeramp-toml-cover.png), [`year`](artifacts/mvp/exp-typeramp-toml-year.png), [`month`](artifacts/mvp/exp-typeramp-toml-month.png), [`week`](artifacts/mvp/exp-typeramp-toml-week.png), [`daily`](artifacts/mvp/exp-typeramp-toml-daily.png), [`override crop`](artifacts/mvp/exp-typeramp-toml-override-header.png). `family="jost"` stays on the ink for a later dual-font ramp.

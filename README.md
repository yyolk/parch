...
Vendored [Jost](https://indestructibletype.com/Jost.html) (Book / Medium / Bold / Heavy) is SIL OFL 1.1 — see `src/parch/fonts/LICENSE`. Weights stay curated. `JostRamp` is a size→weight band table (`≥30` Heavy, `≥11` Medium, else Book) plus a small role set (`cover_year`, `page_title`, `chrome`). Painters pass size (+ optional role / bold); `family="jost"` stays on the ink for a later dual-font ramp.

Thesis F size-band specimens (150 PPI): [`artifacts/mvp/exp-typeramp-sizeband-cover.png`](artifacts/mvp/exp-typeramp-sizeband-cover.png), [`year`](artifacts/mvp/exp-typeramp-sizeband-year.png), [`month`](artifacts/mvp/exp-typeramp-sizeband-month.png), [`week`](artifacts/mvp/exp-typeramp-sizeband-week.png), [`daily`](artifacts/mvp/exp-typeramp-sizeband-daily.png), [`projects`](artifacts/mvp/exp-typeramp-sizeband-projects.png).

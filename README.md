...
Vendored [Jost](https://indestructibletype.com/Jost.html) (Book / Medium / Bold / Heavy) is SIL OFL 1.1 — see `src/parch/fonts/LICENSE`. Weights stay curated. Cover/header roles resolve through `JostRamp.ink`; body painters use `TypeSlot` (`copy` / `mark`) via `ramp.resolve_slot`. `family="jost"` is on the ink for a later dual-font ramp. Thesis K specimens: [`artifacts/mvp/exp-typeramp-slots-cover.png`](artifacts/mvp/exp-typeramp-slots-cover.png), [year](artifacts/mvp/exp-typeramp-slots-year.png), [month](artifacts/mvp/exp-typeramp-slots-month.png), [week](artifacts/mvp/exp-typeramp-slots-week.png), [daily](artifacts/mvp/exp-typeramp-slots-daily.png), [projects](artifacts/mvp/exp-typeramp-slots-projects.png).

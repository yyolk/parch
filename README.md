...
Vendored [Jost](https://indestructibletype.com/Jost.html) (Book / Medium / Bold / Heavy) is SIL OFL 1.1 — see `src/parch/fonts/LICENSE`. Weights stay curated. Closed type roles resolve through `JostRamp`; `family="jost"` is on the ink for a later dual-font ramp. See `src/parch/fonts/README.md` for the role table.

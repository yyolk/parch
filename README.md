...
Default spec is year 2026, Monday week start, **full year** (Jan–Dec). Press walks cover → annual → **projects** board → **projects index** + one sheet per slot → Q1–Q4 quarters → each month **plus its habit tracker** → each ISO week that touches the year (once) → that week’s pressed days + `notes_pages` wells. Projects is exploratory: dest `projects-{year}` is still the three-card board; dest `projects-index-{year}` is Thesis K’s 2×3 mini-cover grid (optional `[projects] index_slots` 6 or 8). Each cover is a 2×2: lined write-in well (UL) plus three empty card frames, and links to `projects-{year}-NN`. Optional `[projects] cards` (2–4, default 3) and `tasks` (3–6, default 4). The quarter page is **A″**: short year-density three minis, then content-height Focus over flex Notes. Bottom nav is Year · Quar · Mon · Habit · **Proj** · Week · Day · Notes (QUAR is provisional). **Proj** lands on the year index. Habit lands on that context’s month tracker. A month header **Habits** chip is a shortcut to the same page; a project sheet header **Index** chip returns to the covers. DAY/NOTES land on the current day (daily/notes), the first pressed day of a week, the 1st of a month, or Jan 1 from the year page — not a press-time “today”. All twelve months’ days and headers are linked. `examples/mvp.toml` uses `notes_pages = 1` to keep the artifact smaller; `2` still works. Optional `[habits] columns = 10`.

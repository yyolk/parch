...
from parch.specimen import (
    ...
    catalog_index_html,
    dotgrid_dests,
    dotgrid_notebook_dests,
    dotgrid_notebook_page_numbers,
    dotgrid_notebook_specimen_spec,
    dotgrid_page_numbers,
    dotgrid_specimen_spec,
    lined_dests,
    lined_dotgrid_notebook_dests,
    lined_dotgrid_notebook_page_numbers,
    lined_dotgrid_notebook_specimen_spec,
    lined_notebook_dests,
    lined_notebook_page_numbers,
...
def test_gallery_groups_split_optional_extras():
    ...
    assert groups["lined-dot-grid-mix-notebook"] == (
...
def test_specimen_index_html_section_anchors():
    ...
    assert html.index('id="lined-notebook"') < html.index(
        'id="lined-dot-grid-mix-notebook"'
    )
    assert html.index('id="lined-dot-grid-mix-notebook"') < html.index('id="steno-pad"')
...
def test_lined_notebook_dests_and_pages():
    ...
    assert spec.device == "kindle-scribe"
    assert spec.lined_sheets == 1
    dests = lined_notebook_dests(spec)
    assert dests["lined-cover"] == spec.cover_dest == "cover"
    assert dests["lined-page"] == spec.dest_for_lined_pad(1) == "lined-2026-01"
    numbers = lined_notebook_page_numbers(spec)
    assert set(numbers) == set(LINED_NOTEBOOK_STEMS)
    assert numbers["lined-cover"] == 1
    assert numbers["lined-page"] == 2
...
def test_lined_dotgrid_notebook_dests_and_pages():
    ...
    assert numbers["lined-dotgrid-cover"] == 1
    assert numbers["lined-dotgrid-lined"] == 2
    assert numbers["lined-dotgrid-dotgrid"] == 3
...
def test_dotgrid_notebook_dests_and_pages():
    ...
    assert dests["dotgrid-cover"] == spec.cover_dest == "cover"
    assert dests["dotgrid-page"] == spec.dest_for_dotgrid_pad(1) == "dotgrid-2026-01"
    numbers = dotgrid_notebook_page_numbers(spec)
    assert set(numbers) == set(DOTGRID_NOTEBOOK_STEMS)
    assert numbers["dotgrid-cover"] == 1
    assert numbers["dotgrid-page"] == 2
...
def test_write_specimens_presses_notebooks_and_steno(tmp_path: Path, monkeypatch):
    ...
    assert any(spec.book == "lined-dot-grid-mix-notebook" for spec in presses)

...
def _year_dests(*, notes_pages: int) -> list[str]:
    ...
    dests.extend(
        spec.dest_for_tasks_index(page)
        for page in range(1, spec.task_index_pages + 1)
    )
    dests.extend(spec.dest_for_task(slot) for slot in range(1, spec.task_count + 1))
...
def test_book_records_year_dests_and_links():
    ...
    assert "tasks-2026-08" in dests
    assert "tasks-2026-08" in links

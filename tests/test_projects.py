...
def test_projects_page_after_annual():
    ...
    assert pages[12].dest == "project-10"
...
def test_projects_knobs_from_spec():
    ...
    assert leaf.tasks == 5
    board = ProjectsBoard(year=2026, cards=2, tasks=5)
    plotter = RecordingPlotter()
...
def test_projects_index_table_tracks():
    well = well_rect(NOMAD)
    head, seats = projects_index_table(well, 10)
    assert head.y == pytest.approx(well.y)
    assert head.h == pytest.approx(PROJECT_TABLE_HEAD_H)
    assert len(seats) == 10
    assert seats[0].y == pytest.approx(head.bottom)
...
def test_paint_projects_index_table_prints_names_and_links():
    ...
    well = well_rect(NOMAD)
    plotter = RecordingPlotter()
    paint_projects_index_table(plotter, well, index)
...
def test_project_index_rows_knob():
    ...
    pages = YearPlanner().pages(spec)
    index_page = next(p for p in pages if p.kind == "projects")
    index = next(item for item in index_page.components if isinstance(item, ProjectsIndex))

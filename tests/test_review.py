...
from parch.layouts.planner.painters import (
    ...
    paint_review,
    paint_review_index,
    review_accomp_row_count,
    review_index_link_hits,
    review_index_week_parts,
    review_index_week_rows,
    review_index_week_strip,
    review_seats,
    strip_active,
    strip_items,
)
...
def test_review_index_page():
    ...
    assert strip_items(page) == _REVIEW_STRIP
...
def test_review_dest_seats():
    well = well_rect(NOMAD)
    n = review_accomp_row_count(well)
    target = (well.h - REVIEW_GAP) * REVIEW_ACCOMP_FRAC
    usable = target - FOCUS_PAD_TOP - FOCUS_PAD_BOT
    expected = max(1, int((usable - TICK) / FOCUS_PITCH) + 1)
    assert n == expected
    assert n == 13
    assert n < 17
    accomp, carry, grateful = review_seats(well)
    assert accomp.y == pytest.approx(well.y)
    assert accomp.h == pytest.approx(checklist_content_height(n, labeled=False))
    assert accomp.w == pytest.approx(well.w)
    assert carry.y == pytest.approx(accomp.bottom + REVIEW_GAP)
    assert grateful.y == pytest.approx(carry.y)
    assert carry.bottom == pytest.approx(well.bottom)
    assert grateful.bottom == pytest.approx(well.bottom)
    assert grateful.x == pytest.approx(carry.right + REVIEW_GAP)
    assert grateful.right == pytest.approx(well.right)
    assert accomp.h <= target
    assert target - accomp.h < FOCUS_PITCH
    assert accomp.h == pytest.approx(well.h * REVIEW_ACCOMP_FRAC, rel=0.08)
    assert carry.h == pytest.approx((well.h - REVIEW_GAP) * (1 - REVIEW_ACCOMP_FRAC), rel=0.12)
    assert carry.w == pytest.approx(grateful.w)
    assert carry.h == pytest.approx(grateful.h)


def test_review_index_seats_equal_week_rows():
    well = well_rect(NOMAD)
    lines = review_index_week_rows(well, 14)
    assert len(lines) == 14
    assert lines[0].x > well.x
    assert lines[-1].bottom < well.bottom
    assert lines[1].y > lines[0].bottom
    stub, dated = review_index_week_parts(lines[0])
    strip = review_index_week_strip(lines[0])
    ...
    assert dated.w == pytest.approx(REVIEW_INDEX_RANGE_W)
    assert review_index_link_hits(lines[0]) == (stub, dated)


def test_review_index_paint_week_links():
    spec = Spec(notes_pages=1, months=(1, 2, 3))
    page = next(p for p in ReviewSection(spec).pages() if p.kind == "review_index")
    index = next(item for item in page.components if isinstance(item, ReviewIndex))
    well = well_rect(NOMAD)
    ...
    seats = review_index_week_rows(well, len(index.weeks))
...
def test_review_paint_accomp_carry_grateful():
    dest = ReviewWeekPage(
        year=2026,
        iso_year=2026,
        iso_week=1,
    ...
    texts = [op[2] for op in plotter.ops if op[0] == "text"]
    assert texts.count("Carry forward") == 1
    assert texts.count("Grateful for") == 1
    assert texts.count("Accomplishments") == 0
    ...
    assert len(ticks) == review_accomp_row_count(well) + dest.carry_rows
    assert review_accomp_row_count(well) > dest.carry_rows
...
def test_review_carry_rows_knob():
    spec = Spec(notes_pages=1, months=(1,), review_carry_rows=4)
    pages = ReviewSection(spec).pages()
    dests = [page.dest for page in pages]
    assert dests[0] == "review-index-2026-Q1"
    assert dests[1:] == [f"review-2026-W{week:02d}" for week in range(1, 6)]
    dest = next(item for item in pages[1].components if isinstance(item, ReviewWeekPage))
    assert dest.carry_rows == 4
    well = well_rect(NOMAD)
    plotter = RecordingPlotter()
    paint_review(plotter, well, dest)
    ticks = [
    ...
    assert len(ticks) == review_accomp_row_count(well) + 4
...
def test_q1_weeks_match_calendar():
    ...
    assert index.weeks[0].monday == calendar[0][1][0][0]

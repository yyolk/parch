...
def test_cli_load_keeps_toml_overlay_under_month_flag():
    ...
    assert spec.type_overlay.cover_year == TypePatch(size=48, weight="heavy")
...
def test_cli_rejects_unknown_typography(tmp_path: Path, capsys):
    ...
    assert "unknown typography overlay role 'body'" in err
...
def test_cli_unknown_weight_fails(tmp_path: Path, capsys):
    ...
    assert "unknown Jost weight 'hairline'" in capsys.readouterr().err

...
from parch.layouts.planner.painters import (
    ...
    paint_project_sheet,
    paint_projects,
    paint_projects_index_grid,
    projects_index_card_seats,
    projects_index_grid,
    strip_active,
    strip_items,
)
...
def test_projects_index_grid_tracks():
    well = Rect(4, 20, 110, 90)
    seats = projects_index_grid(well, 6)
    assert len(seats) == 6
...
def test_paint_projects_index_grid_cards_and_links():
    ...
    paint_projects_index_grid(plotter, well_rect(NOMAD), index)

    texts = [op[2] for op in plotter.ops if op[0] == "text"]
    assert texts.count("P") == 6
    assert texts.count("Todo") == 6
    assert texts.count("Doing") == 6
    assert texts.count("Done") == 6
    assert "PROJECT" not in texts

    p_boxes = [
        op
        for op in plotter.ops
        if op[0] == "rect" and op[2] and not op[3] and op[1].w == pytest.approx(INDEX_P)
    ]
    assert len(p_boxes) == 6
    marks = [
        op
        for op in plotter.ops
        if op[0] == "rect"
...
def test_index_slots_eight_is_two_by_four():
    ...
    seats = projects_index_grid(well, 8)
    assert len(seats) == 8
    plotter = RecordingPlotter()
    paint_projects_index_grid(plotter, well, index)
    assert [op[2] for op in plotter.ops if op[0] == "text"].count("P") == 8
...
def test_index_header_projects_and_year():
    ...
    assert "Proj" in texts
    for dest in (f"projects-2026-{n:02d}" for n in range(1, 7)):
        assert dest in plotter.links()

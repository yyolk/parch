...
def test_checklist_page_and_leaves():
    ...
    assert leaves[0].title == "Project 01"
...
def test_checklist_tracks():
    ...
    tick, name, page = projects_index_checklist_row(seats[0])
...
def test_paint_checklist_writein_pages_ticks_and_links():
    spec = Spec(notes_pages=1)
    ...
    texts = [op[2] for op in plotter.ops if op[0] == "text"]
    for sample in ("Atlas", "Beacon", "Well", "Nomad", "Parch"):
        assert sample not in texts

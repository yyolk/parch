"""Thesis G — strangler allowlist. Listed painters must not emit face-only text."""
...
def test_bridge_backlog_still_emits_face_only(dest, painter, typ):
    page = _page(dest)
    ink = RecordingPlotter()
    painter(ink, well_rect(NOMAD), _one(page, typ), ramp=JostRamp())
...
def test_task_dest_stays_off_allowlist():
    """Weekly Tasks dest is backlog: not listed, even if shared notes use a role helper."""

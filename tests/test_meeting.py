...
def test_meeting_paint_template():
    ...
    paint_meeting(plotter, well, agenda, ramp=JostRamp())
...
def test_meeting_knobs():
    ...
    plotter = RecordingPlotter()
    paint_meeting(plotter, Rect(4, 20, 110, 90), agenda, ramp=JostRamp())
    ticks = [
        op
        for op in plotter.ops
        if op[0] == "rect" and op[2] and not op[3] and op[1].w == pytest.approx(TICK)
...
def test_meeting_index_paint_date_cues_and_links():
    ...
    well = well_rect(NOMAD)
    plotter = RecordingPlotter()
    paint_meetings_index(plotter, well, roster, ramp=JostRamp())
...
def test_meeting_rows_knob():
    ...
    plotter = RecordingPlotter()
    paint_meetings_index(plotter, well_rect(NOMAD), roster, ramp=JostRamp())
    texts = [op[2] for op in plotter.ops if op[0] == "text"]

...
def test_section_emits_duplex_pair_per_sheet_lined_front():
    spec = Spec(lined_dot_grid_sheets=2)
    ...
    assert [page.dest for page in pages] == [
        "lined-dot-grid-2026-01-front",
        "lined-dot-grid-2026-01-back",
        "lined-dot-grid-2026-02-front",
        "lined-dot-grid-2026-02-back",
    ]
...
def test_section_emits_flipped_pair_dotgrid_front():
    ...
    pages = LinedDotGridPadSection(spec, order="dot-grid-lined").pages()
    assert [page.kind for page in pages] == ["dotgrid", "lined"]
    assert [page.dest for page in pages] == [
        "dot-grid-lined-2026-01-front",
        "dot-grid-lined-2026-01-back",
    ]
...
def test_section_empty_when_no_sheets():
    ...
    assert LinedDotGridPadSection(Spec(), order="dot-grid-lined").pages() == []


def test_spec_duplex_dests_and_toml():
    spec = Spec(lined_dot_grid_sheets=1)
    assert spec.dest_for_duplex_pair_pad("lined-dot-grid", 1, "front") == (
        "lined-dot-grid-2026-01-front"
    )
    assert spec.dest_for_duplex_pair_pad("lined-dot-grid", 1, "back") == (
        "lined-dot-grid-2026-01-back"
    )
    example = Spec.from_path(Path("examples/lined-dot-grid-pad.toml"))
    assert example.lined_dot_grid_sheets == 1
    assert example.dot_grid_lined_sheets == 0
    assert example.lined_sheets == 0
    assert example.dotgrid_sheets == 0
    assert example.book == "year-planner"
    assert "year =" not in Path("examples/lined-dot-grid-pad.toml").read_text()
    flipped = Spec.from_path(Path("examples/dot-grid-lined-pad.toml"))
    assert flipped.dot_grid_lined_sheets == 1
    assert flipped.lined_dot_grid_sheets == 0
    assert (
        Spec.from_mapping({"lined-dot-grid": {"sheets": 2}}).lined_dot_grid_sheets == 2
    )
    assert (
        Spec.from_mapping({"dot-grid-lined": {"sheets": 3}}).dot_grid_lined_sheets == 3
    )
    with pytest.raises(ConfigError, match="lined_dot_grid_sheets must be 0–100"):
        Spec(lined_dot_grid_sheets=101)
    with pytest.raises(ConfigError, match="dot_grid_lined_sheets must be 0–100"):
        Spec(dot_grid_lined_sheets=101)
    with pytest.raises(ConfigError, match="front or back"):
        spec.dest_for_duplex_pair_pad("lined-dot-grid", 1, "verso")
    with pytest.raises(ConfigError, match="lined-dot-grid or dot-grid-lined"):
        spec.dest_for_duplex_pair_pad("lined-dotgrid", 1, "front")
    with pytest.raises(ConfigError, match="lined_dot_grid_sheets must be >= 1"):
...
def test_year_planner_rejects_duplex_mixed_with_other_pads(kwargs):
    with pytest.raises(
        ConfigError,
        match="year-planner cannot mix duplex lined/dot-grid sheets with other",
    ):
        Spec(lined_dot_grid_sheets=1, **kwargs)
...
def test_layout_flipped_front_is_dotgrid_back_is_lined():
    spec = Spec(dot_grid_lined_sheets=1)
    front, back = LinedDotGridPadSection(spec, order="dot-grid-lined").pages()
    front_ink = RecordingPlotter()
    back_ink = RecordingPlotter()
    PlannerLayout().paint(front, front_ink, NOMAD)
    PlannerLayout().paint(back, back_ink, NOMAD)
...
def test_press_example_toml_is_duplex_pair_without_cover(tmp_path: Path):
    out = tmp_path / "lined-dot-grid-pad.pdf"
    spec = Spec.from_path(Path("examples/lined-dot-grid-pad.toml"))
    press(spec, out)
    ...
    assert spec.dest_for_duplex_pair_pad("lined-dot-grid", 1, "back") in dests

...
def test_dest_names_from_tstrings():
    ...
    assert spec.projects_index_checklist_dest == "projects-index-checklist-2026"
    ...
    assert spec.project_index_rows == 12
...
def test_habit_columns_from_toml_keys():
    ...
    assert Spec.from_mapping({"projects": {"index_rows": 10}}).project_index_rows == 10
    mvp = Spec.from_path(Path("examples/mvp.toml"))
    assert mvp.project_cards == 3
    assert mvp.project_tasks == 4
    assert mvp.project_index_rows == 12
...
def test_project_index_rows_range():
    with pytest.raises(ConfigError, match="project_index_rows"):
        Spec(project_index_rows=9)
    with pytest.raises(ConfigError, match="project_index_rows"):
        Spec(project_index_rows=13)
    with pytest.raises(ConfigError, match="project dest number"):
        Spec().dest_for_project(0)

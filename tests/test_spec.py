...
def test_dest_names_from_tstrings():
    ...
    assert spec.dest_for_project(6) == "projects-2026-06"
    assert spec.project_cards == 3
    assert spec.project_tasks == 4
    assert spec.project_index_slots == 6
    assert spec.project_titles == tuple(f"Project {n:02d}" for n in range(1, 7))
...
def test_habit_columns_from_toml_keys():
    ...
    assert Spec.from_mapping({"projects": {"index_slots": 8}}).project_index_slots == 8
    named = Spec.from_mapping({"projects": {"titles": ["Atlas", "Harbor", "Keel", "Nomad", "Quarry", "Ridge"]}})
    assert named.project_titles == ("Atlas", "Harbor", "Keel", "Nomad", "Quarry", "Ridge")
    mvp = Spec.from_path(Path("examples/mvp.toml"))
    assert mvp.project_cards == 3
    assert mvp.project_tasks == 4
    assert mvp.project_index_slots == 6


def test_project_titles_must_be_printed_names():
    with pytest.raises(ConfigError, match="printed names"):
        Spec.from_mapping({"projects": {"titles": "Atlas"}})
    with pytest.raises(ConfigError, match="6 names"):
        Spec.from_mapping({"projects": {"titles": ["Atlas"]}})
    with pytest.raises(ConfigError, match="not blank"):
        Spec.from_mapping({"projects": {"titles": ["Atlas", "Harbor", "Keel", "Nomad", "Quarry", ""]}})

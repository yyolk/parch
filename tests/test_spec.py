...
def test_dest_names_from_tstrings():
    ...
    assert spec.dest_for_engineering_pad_front(1) == "engineering-pad-2026-01-front"
    assert spec.dest_for_engineering_pad_back(2) == "engineering-pad-2026-02-back"
    with pytest.raises(ConfigError, match="engineering pad sheet out of range"):
        spec.dest_for_engineering_pad_front(3)
...
def test_habit_columns_from_toml_keys():
    ...
    assert Spec.from_mapping({"book": "engineering-pad"}).book == "engineering-pad"
    assert (
        Spec.from_mapping({"engineering_pad": {"sheets": 4}}).engineering_pad_sheets
        == 4
    )
    with pytest.raises(ConfigError, match="book must be"):
        Spec.from_mapping({"book": "meetings-notebook"})
    with pytest.raises(ConfigError, match="engineering_pad_sheets"):
        Spec.from_mapping({"engineering_pad": {"sheets": 0}})

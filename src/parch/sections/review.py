...
from parch.components import ReviewIndex, ReviewWeek, ReviewWeekPage
...
class ReviewSection:
    """Exploratory Review dest D + minimal quarter index. Not in YearPlanner."""
    ...
    def pages(self) -> list[Page]:
        ...
        for month, weeks in month_week_bands(spec.year, spec.months, spec.weekday_start):
            for week in weeks:
                by_quarter.setdefault(quarter_of(month), []).append(self._review_week(week))
        for quarter in spec.pressed_quarters():
            ...
            index_dest = spec.dest_for_review_index(quarter)
            built.append(
                Page(
                    dest=index_dest,
                    kind="review_index",
        ...
        return Page(
            ...
            components=(
                ReviewWeekPage(
                    ...
                    monday=week.monday,
                    sunday=week.sunday,
                    carry_rows=spec.review_carry_rows,

...
from parch.components import ReviewWeek, ReviewsIndex, ReviewWeekPage
...
class ReviewSection:
    """Exploratory Review dest A + quarter index stubs. Not in YearPlanner."""
    ...
    def pages(self) -> list[Page]:
        ...
        for month, weeks in month_week_bands(spec.year, spec.months, spec.weekday_start):
            rows = [self._review_week(week) for week in weeks]
            by_quarter.setdefault(quarter_of(month), []).extend(rows)
        for quarter in spec.pressed_quarters():
            weeks = tuple(by_quarter.get(quarter, ()))
            if not weeks:
                continue
            landing = self._landing(weeks[0].monday)
            ...
            built.append(
                Page(
                    ...
                    nav=planner_nav(
                        spec,
                        week_dest=spec.dest_for_week(weeks[0].monday),
                        day=landing,
                        month=landing.month,
                        rev_dest=index_dest,
                    ),

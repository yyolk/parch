...
_BOOK_CHOICES = (
    "year-planner, projects-notebook, engineering-notebook, bullet-journal, or dot-grid"
)
...
class Spec:
    ...
    dot_grid_sheets: int = 0  # coverless full-bleed sheets; 0 keeps year-planner press
    ...
    def __post_init__(self) -> None:
        ...
        if self.book == "dot-grid" and self.dot_grid_sheets < 1:
            raise ConfigError("dot-grid requires dot_grid_sheets >= 1")
        if self.book == "dot-grid" and self.engineering_sheets:
            raise ConfigError("dot-grid cannot set engineering_sheets")
        if self.book == "dot-grid" and self.steno_sheets:
            raise ConfigError("dot-grid cannot set steno_sheets")
        if not self.months:
    ...
    def dest_for_dot_grid(self, sheet: int) -> str:
        """1-based coverless dest, e.g. ``dot-grid-2026-01``."""
        if self.dot_grid_sheets < 1:
            raise ConfigError("dot_grid_sheets must be >= 1 to name a sheet dest")

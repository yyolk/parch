...
class Spec:
    ...
    def __post_init__(self) -> None:
        ...
        if not 6 <= self.project_index_rows <= 8:
            raise ConfigError("project_index_rows must be 6–8")
    ...
    def project_page_count(self) -> int:
        """Featured slot + compact index rows."""
        return 1 + self.project_index_rows
    ...
    def from_mapping(cls, data: TomlTable) -> Spec:
        ...
        return cls(
            ...
            project_index_rows=int(
                projects_table.get("index_rows", data.get("project_index_rows", 7))
            ),
        )

...
class Spec:
    ...
    def __post_init__(self) -> None:
        ...
        if not 10 <= self.project_index_rows <= 12:
            raise ConfigError("project_index_rows must be 10–12")
    ...
    @property
    def projects_index_checklist_dest(self) -> str:
        return _dest(t"projects-index-checklist-{self.year:04d}")

    def dest_for_project(self, number: int) -> str:
        """1-based individual project dest, e.g. ``project-2026-01``."""
        if number < 1:
            raise ConfigError(f"project dest number must be >= 1, not {number}")
        return _dest(t"project-{self.year:04d}-{number:02d}")
    ...
    def from_mapping(cls, data: TomlTable) -> Spec:
        ...
        return cls(
            ...
            project_index_rows=int(
                projects_table.get("index_rows", data.get("project_index_rows", 12))
            ),
        )

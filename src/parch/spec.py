...
class Spec:
    ...
    def __post_init__(self) -> None:
        ...
        if not 3 <= self.meeting_index_rows <= 5:
            raise ConfigError("meeting_index_rows must be 3–5")
    ...
    @property
    def meeting_band_count(self) -> int:
        """Thesis C week bands: This week / Next week / Later."""
        return 3

    @property
    def meeting_count(self) -> int:
        """Meeting dest pages: ``bands × index_rows`` (one row → one dest)."""
        return self.meeting_band_count * self.meeting_index_rows

    @property
    def meetings_index_dest(self) -> str:
        """MEET landing — week-banded index."""
        return _dest(t"meetings-index-weeks-{self.year:04d}")
    ...
    def from_mapping(cls, data: TomlTable) -> Spec:
        ...
        return cls(
            ...
            meeting_index_rows=int(
                meetings_table.get("index_rows", data.get("meeting_index_rows", 4))
            ),
        )

...
class Spec:
    ...
    def __post_init__(self) -> None:
        ...
        if not 3 <= self.project_index_rows <= 5:
            raise ConfigError("project_index_rows must be 3–5")
    ...
    @property
    def projects_index_bands_dest(self) -> str:
        return _dest(t"projects-index-bands-{self.year:04d}")

    def dest_for_project(self, index: int) -> str:
        if index < 1:
            raise ConfigError(f"project dest index must be >= 1, not {index}")
    ...
    def from_mapping(cls, data: TomlTable) -> Spec:
        ...
        return cls(
            ...
            project_index_rows=int(
                projects_table.get("index_rows", data.get("project_index_rows", 4))
            ),
        )

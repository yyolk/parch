...
class Spec:
    ...
    def __post_init__(self) -> None:
        ...
        if not 6 <= self.task_tickets <= 10:
            raise ConfigError("task_tickets must be 6–10")
    ...
    def task_count(self) -> int:
        """Weekly Tasks dest pages: ``index_pages × tickets`` (one row → one week template)."""
        return self.task_index_pages * self.task_tickets
    ...
    def dest_for_tasks_index_of(self, slot: int) -> str:
        """Index page that lists ``slot`` (1-based global week row)."""
        if not 1 <= slot <= self.task_count:
            raise ConfigError(f"task slot out of range: {slot}")
        page = (slot - 1) // self.task_tickets + 1
        return self.dest_for_tasks_index(page)

    def dest_for_task(self, slot: int) -> str:
        if not 1 <= slot <= self.task_count:
            raise ConfigError(f"task slot out of range: {slot}")
    ...
    def from_mapping(cls, data: TomlTable) -> Spec:
        ...
        return cls(
            ...
            task_tickets=int(
                tasks_table.get(
                    "tickets_per_page",
                    tasks_table.get("tickets", data.get("task_tickets", 8)),
                )
            ),

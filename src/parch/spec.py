...
class Spec:
    ...
    def __post_init__(self) -> None:
        ...
        if not 4 <= self.meeting_covers <= 12:
            raise ConfigError("meeting_covers must be 4–12")
    ...
    def meeting_count(self) -> int:
        """Dest pages: ``index_pages × covers`` (one cover → one meeting dest)."""
        return self.meeting_index_pages * self.meeting_covers
    ...
    def dest_for_meetings_index(self, page: int) -> str:
        ...
        return _dest(t"meetings-index-{self.year:04d}-{page:02d}")
    ...
    def dest_for_meetings_index_of(self, slot: int) -> str:
        ...
        page = (slot - 1) // self.meeting_covers + 1
    ...
    def from_mapping(cls, data: TomlTable) -> Spec:
        ...
        return cls(
            ...
            meeting_covers=int(
                meetings_table.get(
                    "covers_per_page",
                    meetings_table.get("covers", data.get("meeting_covers", 6)),
                )
            ),

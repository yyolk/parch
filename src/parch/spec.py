...
class Spec:
    ...
    dot_sheets: int = 0  # single-face edge-to-edge dots; 0 keeps year-planner press
    ...
    def __post_init__(self) -> None:
        ...
        if self.book == "engineering-notebook" and self.dot_sheets:
            raise ConfigError("engineering-notebook cannot set dot_sheets")
        if not self.months:
        ...
        if not 0 <= self.dot_sheets <= 100:
            raise ConfigError("dot_sheets must be 0–100")
    ...
    def dest_for_dot_pad(self, sheet: int) -> str:
        """1-based single-face dest, e.g. ``dot-2026-01``."""
        if self.dot_sheets < 1:
            raise ConfigError("dot_sheets must be >= 1 to name a pad dest")
        if not 1 <= sheet <= self.dot_sheets:
            raise ConfigError(f"dot sheet out of range: {sheet}")
        return _dest(t"dot-{self.year:04d}-{sheet:02d}")
    ...
    def from_mapping(cls, data: TomlTable) -> Spec:
        ...
        dot = data.get("dot")
        dot_table = dot if isinstance(dot, dict) else {}
        ...
        return cls(
            ...
            dot_sheets=int(dot_table.get("sheets", data.get("dot_sheets", 0))),

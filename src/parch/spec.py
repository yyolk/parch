...
class Spec:
    ...
    steno_pages: int = 0  # single-sided Gregg faces; 0 keeps year-planner press
    ...
    def __post_init__(self) -> None:
        ...
        if not 0 <= self.steno_pages <= 24:
            raise ConfigError("steno_pages must be 0–24")
        if self.engineering_sheets and self.steno_pages:
            raise ConfigError("steno_pages and engineering_sheets cannot both be set")
    ...
    def dest_for_steno(self, sheet: int) -> str:
        ...
        if self.steno_pages < 1:
            raise ConfigError("steno_pages must be >= 1 to name a pad dest")
        if not 1 <= sheet <= self.steno_pages:
    ...
    def from_mapping(cls, data: TomlTable) -> Spec:
        ...
        return cls(
            ...
            steno_pages=int(steno_table.get("pages", data.get("steno_pages", 0))),

...
def press(
    ...
    When any pad sheet count is set, press concatenates those thin
    section wrappers (engineering faces, then Gregg, then dot-grid)
    and walks them with one ``plot_pages`` — no cover, no new Book.
    Engineering faces join when both pad counts are set, or when
    year-planner has ``engineering_sheets``. ``book =
    "engineering-notebook"`` still presses cover + pad faces through
    ``Book``; combining it with ``steno_sheets`` or
    ``dot_grid_sheets`` raises ``ConfigError``.
    """
    ...
    if (ledger := _pad_ledger(spec)) is not None:
        plot_pages(
            lambda: ledger,
            plotter,
            ramp=resolved,
...
def _pad_ledger(spec: Spec) -> list[Page] | None:
    """Pad-only page ledger, or ``None`` to keep Book press.

    Same hijacks as before, plus optional ``DotGridSection`` sheets.
    """
    engineering = spec.engineering_sheets > 0 and (
        spec.steno_sheets > 0 or spec.book == "year-planner"
    )
    if not (engineering or spec.steno_sheets or spec.dot_grid_sheets):
        return None
    pages: list[Page] = []
    if engineering:

...
def press(
    ...
    section only (no steno-notebook book yet). When ``book`` is
    ``year-planner`` and ``spec.engineering_sheets > 0``, press the
    duplex pad section only (legacy pad TOML). ``engineering-notebook``
    is the sibling book (cover + N duplex sheets). Year-planner specs
...
def _plot_engineering_pad(spec: Spec, plotter: Plotter, ramp: TypeRamp) -> None:
    """Plot ``EngineeringPadSection`` faces through the shared ledger walk."""
    plot_pages(EngineeringPadSection(spec).pages(), spec, plotter, ramp=ramp)


def _plot_steno_pad(spec: Spec, plotter: Plotter, ramp: TypeRamp) -> None:
    """Plot ``StenoPadSection`` faces through the shared ledger walk."""
    plot_pages(StenoPadSection(spec).pages(), spec, plotter, ramp=ramp)

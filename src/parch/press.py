...
def press(
    ...
    with no other pad counts presses ``perspective_pages`` alone —
    no cover, no content-frame inset. Mixing perspective with other
    pad counts raises ``ConfigError``. ``book = "perspective-notebook"``
    presses cover + perspective pages through ``Book``.

...
def press(
    ...
    Default ramp is ``JostRamp`` (size-band weights + small role set). The
    ramp's catalog is handed to ``Fpdf2Plotter``. Dual-font ramps are future
    work — ``family`` stays on ``TypeInk`` / ``Plotter.text`` so they can land

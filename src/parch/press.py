...
def press(
    ...
    When ``spec.book == "engineering-notebook"``, press Cover + duplex
    pad (``book_for``). When ``spec.steno_sheets > 0``, press the
    single-face Gregg pad section only (no steno-notebook book yet).
    When ``spec.engineering_sheets > 0`` on another book, press the
    duplex pad section only. Year-planner specs keep both counts at 0.
    """

...
def press(
    ...
    When ``ramp`` is omitted, press **validates** device ⊕ press overlay
    (pure ``validate_overlay``, exact ``schema_version`` match) before
    building ``EffectiveRamp = defaults ⊕ overlays``. A bad overlay raises
    ``ConfigError`` before paint. An explicit ``ramp`` wins the whole object
    (overlay args are ignored). Painters never read the overlay. The ramp's
    ...
    press_overlay = (
        TypeOverlay() if overlay is None else require_overlay(overlay, defaults)
    )
    resolved = bind_ramp(
        ramp=ramp,
        overlay=compose_overlays(device_overlay, press_overlay),
        defaults=defaults,
    )
    if plotter is None:

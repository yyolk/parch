...
def merge_press_overlay(
    ...
    """``code defaults ⊕ device ⊕ toml ⊕ press kwarg`` — later explicit fields win.

    Defaults live in ``EffectiveRamp`` / ``JostRamp``. This function returns
    the composed overlay only.
    """
    return compose_overlays(device.type_overlay, spec.type_overlay, overlay)
...
def press(
    ...
    device overlay ⊕ spec TOML overlay ⊕ press overlay`` and passes that
    explicit object in. An explicit ``ramp`` wins the whole object (overlay
    args are ignored). Painters never read the overlay — they call
    ``ramp.ink``. The ramp's catalog is handed to ``Fpdf2Plotter``. Dual-font
    ramps are future work — ``family`` stays on ``TypeInk`` / ``Plotter.text``.
    """
    device = get_device(spec.device)
    resolved = bind_ramp(ramp=ramp, overlay=merge_press_overlay(device, spec, overlay))
    if plotter is None:

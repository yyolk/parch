...
def main(argv: list[str] | None = None) -> int:
    ...
    # Accept a leading `press`, `proof`, `specimen`, or `init` verb. `parch
    # proof` is the historical on-screen path; it selects ProofProfile. `parch
    # specimen` writes a static PNG catalog (not a product PDF). `parch init`
    # copies a packaged starter TOML via importlib.resources.
    ...
    if raw and raw[0] == "init":
        from parch.starters import main as init_main

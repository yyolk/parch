...
def press(
    ...
    When ``spec.steno_pages > 0``, press the single-page Gregg section
    only (no steno-notebook book). When ``spec.engineering_sheets > 0``,
    press the duplex pad section only (no engineering-notebook book).
    Year-planner specs keep both counts at 0.
    """
    ...
    if spec.steno_pages > 0:
        _plot_section_pages(spec, plotter, resolved, StenoSection(spec).pages())
    elif spec.engineering_sheets > 0:
        _plot_section_pages(
            spec, plotter, resolved, EngineeringPadSection(spec).pages()
        )
    else:
...
def _plot_section_pages(
    spec: Spec, plotter: Plotter, ramp: TypeRamp, pages: list[Page]
) -> None:
    """Plot a pad section's pages. Progress ticks match the books."""
    device = get_device(spec.device)

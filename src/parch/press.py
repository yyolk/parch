...
def press(
    ...
    Default ramp is ``JostRamp(root_body=device.root_body)``. The ramp's
    catalog is handed to ``Fpdf2Plotter``. Dual-font ramps are future work —
    ``family`` stays on ``TypeInk`` / ``Plotter.text`` so they can land
    without a signature change.
    """
    device = get_device(spec.device)
    resolved = JostRamp(root_body=device.root_body) if ramp is None else ramp
    if plotter is None:

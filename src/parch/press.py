...
def press(
    ...
    ``engineering_sheets = 0``. When ``spec.steno.pages > 0``, press the
    single-sided Gregg pad section only (no steno-notebook book).
    """
...
def _plot_steno_pad(spec: Spec, plotter: Plotter, ramp: TypeRamp) -> None:
    """Plot ``StenoPadSection`` sheets. Progress ticks match the books."""
    device = get_device(spec.device)
    layout = PlannerLayout(ramp=ramp)

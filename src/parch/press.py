...
def main(argv: list[str] | None = None) -> int:
    ...
    # Accept a leading `press`, `proof`, `specimen`, or `init` verb. `parch proof` is
    # the historical on-screen path; it selects ProofProfile. `parch specimen`
    # writes a static PNG catalog (not a product PDF). `parch init --template`
    # is a thin copier wrapper (optional extra; see docs/external-template.md).

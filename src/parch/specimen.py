...
# Notebook cover — pressed from EngineeringNotebook (Cover + duplex sheets).
ENGINEERING_NOTEBOOK_STEMS = ("engineering-notebook-cover",)
...
def write_specimens(
    ...
    *,
    ...
    """Press a slim book plus pad faces and the engineering-notebook cover."""
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        notebook_pdf = Path(tmp) / "engineering-notebook.pdf"
        press(
            replace(spec, book="engineering-notebook", engineering_sheets=1),
            notebook_pdf,
            proof=True,
        )
        render_page_png(notebook_pdf, 1, dest / "engineering-notebook-cover.png")
    write_device_index(
        dest,
        spec.device,
        stems=(*stems, *ENGINEERING_STEMS, *ENGINEERING_NOTEBOOK_STEMS),
    )
    return dest

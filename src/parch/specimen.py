...
# Pad-only full-bleed dots (dotgrid_sheets=1).
...
def gallery_groups(
    ...
    return (
        ...
        ("dotgrid-pad", "Dot grid", DOTGRID_STEMS),
    )
...
def dotgrid_specimen_spec(device_id: str, *, year: int = 2026) -> Spec:
    """One full-bleed dot-grid sheet for the catalog."""
    return replace(
...
def dotgrid_page_numbers(
    ...
    return _page_numbers(DotGridSection(spec).pages(), dotgrid_dests(spec), stems)

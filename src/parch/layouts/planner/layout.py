...
class PlannerLayout:
    ...
    def _paint_well(self, page: Page, plotter: Plotter, well: Rect) -> None:
        match page.kind:
            ...
            case "tasks_index":
                paint_tasks_index_roster(plotter, well, _one(page, TasksIndex))
            case "task":
                paint_week_tasks(plotter, well, _one(page, WeekTasks))
            case "quarter":
...
def _header_meta(page: Page) -> str:
    match page.kind:
        ...
        case "tasks_index":
            return str(_one(page, TasksIndex).year)
        case "task":
            return str(_one(page, WeekTasks).year)
        case "quarter":
            return ""
        case "month":
...
def _header_chip(page: Page) -> str:
    match page.kind:
        ...
        case "meeting":
            ...
            return f"{number:02d}" if number else ""
        case "task":
            week = _one(page, WeekTasks)
            return f"W{week.iso_week:02d}" if week.iso_week else ""
        case _:
            return ""
...
def _header_chip_dest(page: Page) -> str | None:
    match page.kind:
        ...
        case "task":
            return _one(page, WeekTasks).index_dest or None
        case _:
            return None

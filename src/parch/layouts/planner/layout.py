...
from parch.fonts.ramp import JostRamp, OverlayStack, TypeOverlay, TypeRamp, bind_ramp
...
class PlannerLayout:
    ...
    defaults, no overlay). Pass ``overlay=``, ``stack=``, or an
    ``EffectiveRamp`` to apply stacked patches. Painters call
    ``ramp.ink(...)``; they never read the overlay. Dual-font ramps are
    ...
    def __init__(
        ...
        self.ramp: TypeRamp = (
            JostRamp()
            if ramp is None and overlay is None and stack is None
            else bind_ramp(ramp=ramp, stack=stack, overlay=overlay)
        )
    ...
    def _paint_well(self, page: Page, plotter: Plotter, well: Rect) -> None:
        match page.kind:
            ...
            case "project":
                paint_project(plotter, well, _one(page, ProjectsBoard), ramp=self.ramp)
            case "meetings_index":
            ...
            case "daily":
                ...
                paint_schedule(plotter, sched_box, schedule, ramp=self.ramp)
                _paint_mini_month(plotter, mini_box, mini, ramp=self.ramp)
                paint_priorities(plotter, prio_box, priorities, ramp=self.ramp)
                paint_notes(plotter, notes_box, notes, ramp=self.ramp)
            case "daily_notes":

...
def paint_projects_index_chips(plotter: Plotter, box: Rect, index: ProjectsIndex) -> None:
    ...
    for seat, chip in zip(
        ...
        plotter.rect(seat, stroke=True, fill=False, stroke_width=HAIR, stroke_gray=INK)
        inner = Rect(seat.x + CHIP_INSET_X, seat.y, seat.w - 2 * CHIP_INSET_X, seat.h)
        glyph = Rect(
            inner.right - CHIP_GLYPH,
            inner.y + (inner.h - CHIP_GLYPH) / 2,
            CHIP_GLYPH,
            CHIP_GLYPH,
        )
...
def _paint_chip_status(plotter: Plotter, mark: Rect, status: str) -> None:
    ...
    if status == "doing":
        plotter.rect(mark.inset(0.7, 0.7), stroke=False, fill=True, fill_gray=INK)
...
def paint_project(plotter: Plotter, box: Rect, page: ProjectPage) -> None:
    ...
    _paint_clone_status_track(plotter, rail, page.status)
    plotter.rect(board, stroke=True, fill=False, stroke_width=HAIR, stroke_gray=INK)
...
def _paint_clone_priority(plotter: Plotter, header: Rect) -> float:
    """P-box: muted corner-fraction label. Clone / G leaf only."""
...
def _paint_clone_status_track(plotter: Plotter, box: Rect, status: str = "todo") -> None:
    """Vertical Todo → In Progress → Done. Active stage is a filled square."""
    track_h = min(CLONE_TRACK_H, box.h - 2.0)
    track = Rect(box.x, box.y + (box.h - track_h) / 2, box.w, track_h)
    inset = track.inset(1.4, 0.6)
    marks: list[Rect] = []
    active = {"todo": 0, "doing": 1, "done": 2}.get(status, 0)
    for i, (slot, label) in enumerate(
        zip(rows(inset, 3, gap=1.8), CLONE_STATUS_LABELS, strict=True)
    ):
        mark_y = slot.y + (slot.h - PROJECT_STATUS_MARK) / 2
        mark = Rect(slot.x, mark_y, PROJECT_STATUS_MARK, PROJECT_STATUS_MARK)
        if i == active:

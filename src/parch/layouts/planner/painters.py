...
def _text(
    ...
    *,
    ...
    """Draw with pack ink only — never face/bold, never a role name."""
    plotter.text(
        box,
        content,
        size=ink.size,
...
def paint_header(
    ...
    *,
    pack: BaseChrome | None = None,
    chip: str = "",
    chip_dest: str | None = None,
) -> None:
    chrome = default_packs().annual.chrome if pack is None else pack
    ...
    _text(plotter, title_box, title, chrome.title, gray=PAPER, align="left")
    if chip:
...
def paint_nav(
    ...
    active: str,
    *,
    pack: NavPack | None = None,
) -> None:
    nav = default_packs().annual.chrome.nav if pack is None else pack
    if not items:
        return
    ...
    for i, (label, dest) in enumerate(items):
        ...
        _text(
            ...
            hit,
            label,
            nav.active if on else nav.idle,
...
def paint_cover(
    plotter: Plotter, device: Device, cover: CoverTitle, *, pack: CoverPack | None = None
) -> None:
    pack = default_packs().cover if pack is None else pack
    ...
    _text(plotter, year_box, str(cover.year), pack.year, gray=INK, align="center")
    tap_w = 48.0
    plotter.link(
...
def paint_annual(
    plotter: Plotter, box: Rect, grid: AnnualGrid, *, pack: AnnualPack | None = None
) -> None:
    pack = default_packs().annual if pack is None else pack
    for r, band in enumerate(rows(box, 4, gap=2.6)):
        for c, cell in enumerate(columns(band, 3, gap=3.4)):
            _paint_mini_month(plotter, cell, grid.months[r * 3 + c], pack=pack.mini)
...
def _paint_project_ticket(
    ...
    _text(plotter, mark, f"{ticket.number:02d}", pack.stub, gray=INK, align="center")
...
def paint_project(
    plotter: Plotter, box: Rect, board: ProjectsBoard, *, pack: ProjectsPack | None = None
) -> None:
    ...
    for card, rail in zip(cards, rails, strict=True):
        ...
        _paint_clone_priority(plotter, name_h, pack=pack)
        ...
        _paint_clone_icon_strip(plotter, strip)
        _paint_clone_status_track(plotter, rail, pack=pack)
        plotter.rect(card, stroke=True, fill=False, stroke_width=HAIR, stroke_gray=INK)
...
def _paint_clone_priority(plotter: Plotter, header: Rect, *, pack: ProjectsPack) -> float:
...
def _paint_clone_status_track(plotter: Plotter, box: Rect, *, pack: ProjectsPack) -> None:
...
def paint_meeting(
    plotter: Plotter, box: Rect, agenda: MeetingAgenda, *, pack: MeetingPack | None = None
) -> None:
    """Locked Meeting dest — title|date, agenda, notes, action items."""
    pack = default_packs().meeting if pack is None else pack
    head, agenda_box, notes, action_items = meeting_seats(
        box, agenda.agenda, agenda.action_items
    )
    _paint_meeting_head(plotter, head, pack=pack)
    _paint_checklist_box(plotter, agenda_box, label="Agenda", rows=agenda.agenda, ink=pack.label)
    _paint_note_box(plotter, notes, label="Notes", ink=pack.label)
    _paint_checklist_box(
        plotter, action_items, label="Action items", rows=agenda.action_items, ink=pack.label
    )


def _paint_meeting_head(plotter: Plotter, head: Rect, *, pack: MeetingPack) -> None:
    plotter.rect(head, stroke=True, fill=False, stroke_width=HAIR, stroke_gray=SOFT)
    title, dated = meeting_head_seats(head)
    _paint_meeting_writein(plotter, title, "Title", pack=pack)
    _paint_meeting_writein(plotter, dated, "Date", pack=pack)


def _paint_meeting_writein(
    plotter: Plotter, box: Rect, label: str, *, pack: MeetingPack
) -> None:
    ...
    _text(plotter, label_box, label, pack.label, gray=MUTED, small_caps=True, align="left")
...
def paint_meetings_index(
    plotter: Plotter, box: Rect, index: MeetingIndex, *, pack: MeetingPack | None = None
) -> None:
    ...
    for seat, slot in zip(meetings_index_roster(box, len(index.slots)), index.slots, strict=True):
        _paint_meeting_index_row(plotter, seat, slot.number, pack=pack)
        for hit in meeting_index_link_hits(seat):
            plotter.link(hit, slot.dest)


def _paint_meeting_index_row(
    plotter: Plotter, box: Rect, number: int, *, pack: MeetingPack
) -> None:
    """Slot stub + date cue + title write-in/rule on one baseline."""
    stub, _body = meeting_index_row_parts(box)
    _paint_meeting_index_stub(plotter, stub, number, pack=pack)
    dated, title = meeting_index_row_seats(box)
    _paint_meeting_index_date_cue(plotter, dated, pack=pack)
    _paint_meeting_index_title(plotter, title)


def _paint_meeting_index_stub(
    plotter: Plotter, stub: Rect, number: int, *, pack: MeetingPack
) -> None:
    ...
    plotter.rect(mark, stroke=True, fill=False, stroke_width=HAIR, stroke_gray=INK)
    _text(plotter, mark, f"{number:02d}", pack.stub, gray=INK, align="center")


def _paint_meeting_index_date_cue(
    plotter: Plotter, box: Rect, *, pack: MeetingPack
) -> None:
...
def paint_tasks_index(
    plotter: Plotter, box: Rect, index: TasksIndex, *, pack: TasksPack | None = None
) -> None:
    """Thesis C — month-banded week rows. Not Active/Waiting/Done, not This week/Later."""
    pack = default_packs().tasks if pack is None else pack
    ...
    for band_box, band in zip(tasks_index_bands(box, counts), index.bands, strict=True):
        ...
        _text(plotter, head, band.name, pack.band, gray=INK, small_caps=True, align="left")
        ...
        for line, week in zip(lines, band.weeks, strict=True):
            _paint_tasks_index_week(plotter, line, week, pack=pack)
            for hit in tasks_index_link_hits(line):
                plotter.link(hit, week.dest)


def _paint_tasks_index_week(
    plotter: Plotter, row: Rect, week: TaskWeek, *, pack: TasksPack
) -> None:
    stub, dated, write = tasks_index_week_parts(row)
    _text(plotter, stub, f"W{week.iso_week:02d}", pack.week, gray=INK, align="left")
    _text(
        plotter,
        dated,
...
def paint_task(
    plotter: Plotter, box: Rect, page: TasksWeekPage, *, pack: TasksPack | None = None
) -> None:
    ...
    _paint_checklist_box(plotter, checklist, rows=rows_n, ink=pack.label)
    _paint_note_box(plotter, notes, label="Notes", ink=pack.label)
...
def paint_review_index(
    plotter: Plotter, box: Rect, index: ReviewIndex, *, pack: ReviewPack | None = None
) -> None:
    """Thesis B — dense week chips in several columns; month headers + hairlines."""
    pack = default_packs().review if pack is None else pack
    counts = tuple(len(band.weeks) for band in index.bands)
    ...
    for i, (row, band) in enumerate(zip(bands, index.bands, strict=True)):
        ...
        _text(plotter, stub, band.name, pack.band, gray=INK, small_caps=True, align="left")
        for cell, week in zip(cells, band.weeks, strict=True):
            _paint_review_index_chip(plotter, cell, week, pack=pack)
...
def _paint_review_index_chip(
    plotter: Plotter, cell: Rect, week: ReviewWeek, *, pack: ReviewPack
) -> None:
    chip = review_index_chip(cell)
    plotter.rect(chip, stroke=True, fill=False, stroke_width=HAIR, stroke_gray=SOFT)
    _text(plotter, chip, f"W{week.iso_week:02d}", pack.chip, gray=INK, align="center")
...
def paint_review(
    plotter: Plotter, box: Rect, page: ReviewWeekPage, *, pack: ReviewPack | None = None
) -> None:
    ...
    for cue, day in zip(review_day_cues(strip), page.days, strict=True):
        _paint_review_day_cue(plotter, cue, day, pack=pack)
        if day.dest:
            for hit in review_day_link_hits(cue):
                plotter.link(hit, day.dest)
    _paint_note_box(plotter, notes, ink=pack.band)


def _paint_review_day_cue(
    plotter: Plotter, cue: Rect, day: ReviewDay, *, pack: ReviewPack
) -> None:
    ...
    _text(plotter, num, str(day.day.day), pack.day, gray=ink, align="center")
...
def paint_quarter(
    plotter: Plotter, box: Rect, grid: QuarterGrid, *, pack: QuarterPack | None = None
) -> None:
    """Default quarter seat is A″ — year-density minis, content-height Focus over flex Notes."""
    paint_quarter_a_focus_notes(plotter, box, grid, pack=pack)
...
def paint_quarter_a_shortband(
    ...
    pack = default_packs().quarter if pack is None else pack
    for cell, month in zip(quarter_seats_a_shortband(box), grid.months, strict=True):
        _paint_mini_month(plotter, cell, month, pack=pack.mini)
...
def paint_quarter_a_note_boxes(
    ...
    for (cal, notes), month in zip(quarter_seats_a_note_boxes(box), grid.months, strict=True):
        _paint_mini_month(plotter, cal, month, pack=pack.mini)
        _paint_note_box(plotter, notes, ink=pack.label)
...
def paint_quarter_c_stack_notes(
    ...
    for cell, month in zip(months, grid.months, strict=True):
        _paint_mini_month(plotter, cell, month, pack=pack.mini)
    paint_notes(plotter, notes, Notes(label="Notes"), ink=pack.label)
...
def paint_quarter_c_focus_notes(
    ...
    for cell, month in zip(months, grid.months, strict=True):
        _paint_mini_month(plotter, cell, month, pack=pack.mini)
    _paint_focus_box(plotter, focus, ink=pack.label)
    paint_notes(plotter, notes, Notes(label="Notes"), ink=pack.label)
...
def _paint_note_box(
    plotter: Plotter, box: Rect, *, label: str | None = None, ink: TypeInk | None = None
) -> None:
    ...
    if label:
        header_h = 3.4
        label_ink = default_packs().daily.label if ink is None else ink
        _text(
            plotter,
...
def _paint_focus_box(plotter: Plotter, box: Rect, *, ink: TypeInk | None = None) -> None:
    """Outlined FOCUS checklist — empty ticks + underline. Not a section."""
    _paint_checklist_box(plotter, box, label="Focus", rows=FOCUS_ROWS, ink=ink)
...
def paint_priorities(
    ...
    *,
    pack: DailyPack | None = None,
) -> None:
    pack = default_packs().daily if pack is None else pack
    _paint_checklist_box(
        plotter, box, label=priorities.label, rows=priorities.rows, ink=pack.label
    )
...
def _paint_mini_month(
    plotter: Plotter, box: Rect, month: AnnualMonth, *, pack: MiniMonthPack | None = None
) -> None:
    pack = default_packs().annual.mini if pack is None else pack
    ...
    _text(
        ...
        title,
        month.name[:3],
        pack.month_on if pressed else pack.month,
    ...
    for band, week in zip(rows(body, 6), month.weeks, strict=False):
        for c, cell in enumerate(week):
            ...
            if here:
                ...
                _text(plotter, num, str(cell.day), pack.day_mark, gray=PAPER, align="center")
                continue
            linked = cell.dest is not None
            gray = MUTED if not cell.in_month else (INK if linked else MUTED)
            day_ink = pack.day_on if linked and cell.in_month else pack.day
            _text(plotter, num, str(cell.day), day_ink, gray=gray, align="center")
            if linked:
...
def paint_habit_grid(
    plotter: Plotter, box: Rect, grid: HabitGrid, *, pack: HabitPack | None = None
) -> None:
    """Locked default: days left with weekday, habit columns, pale zebra."""
    paint_habit_grid_transposed(plotter, box, grid, pack=pack)


def paint_habit_grid_rows(
    plotter: Plotter, box: Rect, grid: HabitGrid, *, pack: HabitPack | None = None
) -> None:
    """Habits as rows, days across. Comparison only — not the default."""
    pack = default_packs().habit if pack is None else pack
    ...
    for i, col in enumerate(day_heads):
        _text(plotter, col, str(i + 1), pack.head_day, gray=MUTED, align="center")
...
def paint_month_grid(
    plotter: Plotter, box: Rect, grid: MonthGrid, *, pack: MonthPack | None = None
) -> None:
    pack = default_packs().month if pack is None else pack
    gutter = 8.0
...
def paint_week(
    plotter: Plotter, box: Rect, week: WeekStrip, *, pack: WeekPack | None = None
) -> None:
    pack = default_packs().week if pack is None else pack
    for band, day in zip(rows(box, max(1, len(week.days))), week.days, strict=False):
        gray = INK if day.in_month else MUTED
        _text(
            plotter,
...
def paint_schedule(
    plotter: Plotter, box: Rect, schedule: Schedule, *, pack: DailyPack | None = None
) -> None:
    pack = default_packs().daily if pack is None else pack
    header_h = 3.4
    _text(
...
def paint_notes(
    ...
    *,
    ...
    label_ink = (
        ink if ink is not None else (default_packs().daily if pack is None else pack).label
    )
    header_h = 3.4
    _text(
        plotter,

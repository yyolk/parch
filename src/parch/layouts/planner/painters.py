...
def strip_items(page: Page) -> tuple[tuple[str, str], ...]:
    ...
    for item in page.nav:
        if item.dest.startswith("year-"):
            ...
            dests["Day"] = item.dest
    seating_view(page.components, page.dest).pin(dests)
    order = (
        "Key",
        "Idx",
        "Fut",
...
def strip_active(page: Page) -> str:
    """Active strip chip for this seating. Bleed seatings are blank."""
    return seating_view(page.components, page.dest).strip

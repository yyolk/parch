...
from parch.fonts.ramp import JostRamp, TypeEmphasis, TypeRamp, TypeSlot
...
def _ramp(ramp: TypeRamp | None) -> TypeRamp:
...
def _slot_text(
    ...
    *,
    ...
    slot: TypeSlot,
    size: float,
    emphasis: TypeEmphasis = TypeEmphasis.REGULAR,
    align: TextAlign = "left",
    gray: float = 0.0,
    small_caps: bool = False,
) -> None:
    """Resolve slot + emphasis + size on the ramp, then draw the ink."""
    ink = _ramp(ramp).resolve_slot(slot, emphasis, size)
    plotter.text(
        box,
        content,
        size=ink.size,
    ...
    *,
    ...
    for i, (label, dest) in enumerate(items):
        ...
        _slot_text(
            ...
            slot=TypeSlot.COPY,
            size=7.6,
            emphasis=TypeEmphasis.STRONG if on else TypeEmphasis.REGULAR,
...
def paint_cover(plotter: Plotter, device: Device, cover: CoverTitle, *, ramp: TypeRamp) -> None:
    ...
    # Cover specs are not a named role — they take the copy slot, not chrome.
    _slot_text(
        plotter,
        specs,
...
def _paint_clone_priority(plotter: Plotter, header: Rect, *, ramp: TypeRamp | None = None) -> float:
...
def _paint_clone_status_track(plotter: Plotter, box: Rect, *, ramp: TypeRamp | None = None) -> None:
...
def _paint_meeting_head(plotter: Plotter, head: Rect, *, ramp: TypeRamp | None = None) -> None:
...
def _paint_meeting_writein(
    plotter: Plotter, box: Rect, label: str, *, ramp: TypeRamp | None = None
) -> None:
...
def _paint_meeting_index_row(
    plotter: Plotter, box: Rect, number: int, *, ramp: TypeRamp | None = None
) -> None:
...
def _paint_meeting_index_stub(
    plotter: Plotter, stub: Rect, number: int, *, ramp: TypeRamp | None = None
) -> None:
...
def _paint_tasks_index_week(
    plotter: Plotter, row: Rect, week: TaskWeek, *, ramp: TypeRamp | None = None
) -> None:
...
def _paint_review_index_chip(
    plotter: Plotter, cell: Rect, week: ReviewWeek, *, ramp: TypeRamp | None = None
) -> None:
...
def _paint_review_day_cue(
    plotter: Plotter, cue: Rect, day: ReviewDay, *, ramp: TypeRamp | None = None
) -> None:
...
def _paint_mini_month(
    ...
    _slot_text(
        ...
        slot=TypeSlot.COPY,
        size=6.4,
        emphasis=TypeEmphasis.STRONG if pressed else TypeEmphasis.REGULAR,
    ...
    for band, week in zip(rows(body, 6), month.weeks, strict=False):
        for c, cell in enumerate(week):
            ...
            _slot_text(
                ...
                slot=TypeSlot.COPY,
                size=5.3,
                emphasis=TypeEmphasis.STRONG if linked and cell.in_month else TypeEmphasis.REGULAR,
                gray=ink,
                align="center",
            )
            if linked:

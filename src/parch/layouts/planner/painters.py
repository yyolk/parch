...
from parch.components.projects import PROJECT_GLYPHS
...
def paint_projects_clone_faithful(
    plotter: Plotter,
    box: Rect,
    board: ProjectsBoard,
    titles: tuple[str, ...] = (),
    ...
    for i, (card, rail) in enumerate(zip(cards, rails, strict=True)):
        ...
        if i < len(titles) and titles[i]:
            _paint_clone_name_title(plotter, name_field, titles[i])
...
def _paint_clone_name_title(plotter: Plotter, field: Rect, title: str) -> None:
    plotter.text(
        Rect(
            field.x + CLONE_TITLE_PAD,
            field.y,
            max(field.w - 2 * CLONE_TITLE_PAD, 1),
            field.h,
        ),
        title,
        size=8.0,
...
def _paint_projects_index_glyph_row(plotter: Plotter, box: Rect, entry: ProjectEntry) -> Rect:
    ...
    if entry.status:
        plotter.text(
            Rect(box.right - INDEX_STATUS_W, box.y, INDEX_STATUS_W, box.h),
            entry.status,
            size=5.8,
            face="sans",
            gray=MUTED,

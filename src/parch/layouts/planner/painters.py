...
def paint_clone_dot_grid(plotter: Plotter, box: Rect) -> None:
    """RULE_C dots on tracks at ``CLONE_DOT_PITCH``. Fills *box* (no extra inset)."""
...
def _paint_clone_dot_grid(plotter: Plotter, box: Rect) -> None:
    """E-ink dot grid — SOFT pocket, then the shared clone-pitch fill."""
    plotter.rect(box, stroke=True, fill=False, stroke_width=HAIR, stroke_gray=SOFT)
    inset = Rect(box.x + 1.1, box.y + 1.2, box.w - 2.2, box.h - 2.4)
    paint_clone_dot_grid(plotter, inset)
...
def paint_dot_grid_pad(
    ...
    *,
    ...
    """Full-bleed clone-pitch dots. No header, nav, frame, or writing_clearance."""
    _bound_ramp(plotter, ramp)
    paint_clone_dot_grid(plotter, device.page_rect())

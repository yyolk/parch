...
def _text(
    ...
    *,
    ...
    """Draw text; weight comes from the ramp (size band or role override)."""
    ink = ramp.ink(size, role=role, bold=bold)
    plotter.text(
        box,
        content,
        size=ink.size,
...
def paint_header(
    ...
    *,
    ...
    _text(plotter, title_box, title, ramp, 11, role="page_title", gray=PAPER, align="left")
    if chip:
...
def _paint_project_ticket(
    ...
    _text(plotter, mark, f"{ticket.number:02d}", ramp, 6.6, bold=True, gray=INK, align="center")
...
def _paint_meeting_index_stub(
    ...
    plotter.rect(mark, stroke=True, fill=False, stroke_width=HAIR, stroke_gray=INK)
    _text(plotter, mark, f"{number:02d}", ramp, 6.2, bold=True, gray=INK, align="center")
...
def _paint_tasks_index_week(
    ...
    _text(plotter, stub, f"W{week.iso_week:02d}", ramp, 7.2, bold=True, gray=INK, align="left")
    _text(
        plotter,
        dated,
...
def paint_review_index(
    plotter: Plotter, box: Rect, index: ReviewIndex, *, ramp: TypeRamp
) -> None:
...
def _paint_review_index_chip(
    ...
    _text(plotter, chip, f"W{week.iso_week:02d}", ramp, 7.0, bold=True, gray=INK, align="center")
...
def _paint_review_day_cue(
    ...
    _text(plotter, num, str(day.day.day), ramp, 9.2, bold=True, gray=ink, align="center")
    ...
    for band, week in zip(rows(body, 6), month.weeks, strict=False):
        for c, cell in enumerate(week):
            ...
            if here:
                ...
                _text(plotter, num, str(cell.day), ramp, 5.3, bold=True, gray=PAPER, align="center")
                continue
...
def paint_habit_grid_rows(
    ...
    for i, col in enumerate(day_heads):
        _text(plotter, col, str(i + 1), ramp, 3.8, gray=MUTED, align="center")

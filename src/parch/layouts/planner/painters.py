...
TASK_INDEX_BAND_INSET_X = 1.8
TASK_INDEX_BAND_INSET_Y = 1.4
...
def tasks_index_chip_rows(n: int) -> int:
    """Packed two-column chip rows for ``n`` weeks."""
    return (max(0, n) + 1) // 2


def tasks_index_bands(well: Rect, week_counts: tuple[int, ...]) -> tuple[Rect, ...]:
    """Month bands weighted by packed chip-row count, not equal status slices."""
    weights = tuple(float(max(1, tasks_index_chip_rows(count))) for count in week_counts)
    return rows(well, len(weights), gap=TASK_INDEX_BAND_GAP, weights=weights)


def tasks_index_chip_seats(box: Rect, n: int) -> tuple[Rect, ...]:
    """Row-major two-column week chips filling a month-band body."""
    if n < 1:
        return ()
    n_rows = tasks_index_chip_rows(n)
    seats: list[Rect] = []
...
def tasks_index_band_seats(band: Rect, n: int) -> tuple[Rect, tuple[Rect, ...]]:
    """Month header over packed two-column week chips, after a quiet inset."""
    inner = band.inset(TASK_INDEX_BAND_INSET_X, TASK_INDEX_BAND_INSET_Y)
    head, rest = inner.split_top(TASK_INDEX_HEAD_H)
    body = Rect(rest.x, rest.y + TASK_INDEX_HEAD_GAP, rest.w, rest.h - TASK_INDEX_HEAD_GAP)
    return head, tasks_index_chip_seats(body, n)
...
def task_index_link_hits(cell: Rect) -> tuple[Rect, ...]:
    ...
    chip, _span = task_index_chip_parts(cell)
    return (chip,)


def paint_tasks_index_bands(plotter: Plotter, box: Rect, index: TasksIndex) -> None:
    """C×F — month bands (C) packing two-column ISO week chips (F)."""
    ...
    for band_box, band in zip(tasks_index_bands(box, counts), index.bands, strict=True):
        ...
        head, chips = tasks_index_band_seats(band_box, len(band.weeks))
        plotter.text(
            head,
            band.name,
            size=6.4,
        ...
        for cell, week in zip(chips, band.weeks, strict=True):
            _paint_task_index_chip(
                plotter, cell, week.iso_week, short_date_range(week.monday, week.sunday)
            )
            for hit in task_index_link_hits(cell):
...
def paint_weekly_tasks(plotter: Plotter, box: Rect, tasks: WeeklyTasks) -> None:
    """F dest — two-column Morning | Later checklists, leftover notes below."""
